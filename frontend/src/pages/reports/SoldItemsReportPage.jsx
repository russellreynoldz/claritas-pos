...
export default function SoldItemsReportPage({
  ...
  const totalNormalSales = filtered.reduce(
    (sum, item) => sum + Number(item.normal_sales || 0),
    0
  );

  const totalCreditSales = filtered.reduce(
    (sum, item) => sum + Number(item.credit_sales || 0),
    0
  );

  const grandTotalSales = filtered.reduce(
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-0.5">
          View normal and credit items sold for this {period}.
        </p>
      </div>
              ...
              {loading ? (
                ...
                filtered.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3 font-bold text-emerald-700">
                      {fmt(Number(item.normal_sales))}
                    </td>

                    <td className="px-5 py-3 font-bold text-amber-600">
                      {fmt(Number(item.credit_sales))}
                    </td>
