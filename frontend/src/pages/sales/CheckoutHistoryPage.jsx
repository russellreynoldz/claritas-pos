...
export default function CheckoutHistoryPage() {
  ...
  return (
    <div className="p-6 space-y-5">
      <div>
        <h1 className="text-2xl font-bold text-slate-800">Checkout History</h1>
      ...
      <div className="flex flex-wrap items-center gap-3 bg-white p-4 rounded-xl shadow">
        <button onClick={() => changeDate(-1)} className="px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200">
          Previous Day
        </button>
        ...
        <button onClick={() => setDate(today)} className="px-4 py-2 rounded-lg bg-green-600 text-white hover:bg-green-700">
          Today
        </button>
      </div>
                ...
                <tbody>
                  {sales.map((sale) => (
                    <tr key={sale.id}>
                      ...
                      <td className="border p-2">{sale.time}</td>
      ...
      <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
        <div className="bg-white w-full max-w-2xl rounded-xl shadow-lg p-6">
            ...
            <p className="text-sm text-slate-500 mb-4">
              Double-clicked transaction: {selectedSale.transaction_no}
            </p>

            <div className="grid grid-cols-2 gap-3 text-sm mb-4">
              <p><b>Transaction:</b> {selectedSale.transaction_no}</p>
              <p><b>Time:</b> {selectedSale.time}</p>
              <p><b>Cashier:</b> {selectedSale.cashier_name || "N/A"}</p>
              <p><b>Total:</b> ₱{Number(selectedSale.total).toFixed(2)}</p>
            </div>
